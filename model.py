...
def draw_history_of_training(path):
    (loss, accuracy) = (
        np.load(path, allow_pickle=True).item().get("loss_change"),
    ...
    accuracy = [
        sum(accuracy[i : i + len_tr]) / len_tr for i in range(0, len(accuracy), len_tr)
    ]
...
def create_indexes(size_axis, center_w_l):
    ...
    coordinates_a = create_axis_indexes(
        size_axis=size_axis[0], center_w_l=center_w_l[0]
    )
    coordinates_b = create_axis_indexes(
        size_axis=size_axis[1], center_w_l=center_w_l[1]
    )
    return coordinates_a, coordinates_b


def convolution_feed_x_l(y_l_minus_1, w_l, conv_params):
    indexes_a, indexes_b = create_indexes(
        size_axis=w_l.shape, center_w_l=conv_params["center_w_l"]
    )
...
def maxpool(y_l, conv_params):
    indexes_a, indexes_b = create_indexes(
        size_axis=conv_params["window_shape"], center_w_l=conv_params["center_window"]
    )
    stride = conv_params["stride"]
    ...
    y_l_mp_to_y_l = np.zeros(
        (1, 1), dtype="<U32"
    )  # матрица для backprop через слой макспулинга
            ...
            if element_exists:
                ...
                if j >= y_l_mp.shape[1]:
                    ...
                    y_l_mp_to_y_l = np.hstack(
                        (y_l_mp_to_y_l, np.zeros((y_l_mp_to_y_l.shape[0], 1)))
                    )
                y_l_mp[i][j] = result
...
def maxpool_back(dEdy_l_mp, y_l_mp_to_y_l, y_l_shape):
    ...
    for i in range(len(dEdy_l_mp)):  # операция выполняется для каждой из feature map
        dEdy_l = np.zeros(
            y_l_shape
        )  # матрица dEdy_l будет далее постепенно заполнятся значениями
        ...
        for k in range(dEdy_l_mp[i].shape[0]):
            for l in range(dEdy_l_mp[i].shape[1]):
                ...
                coordinates = y_l_mp_to_y_l[i][k][
                    l
                ]  # коордианты выглядят так: 2,4 - то есть 2-ая строка и 4-ый столбец
        ...
        list_of_dEdy_l.append(
            dEdy_l
        )  # добавляем получившуюся dEdy_l в лист с остальными feature map
    return list_of_dEdy_l


def convolution_back_dEdw_l(y_l_minus_1, w_l_shape, dEdx_l, conv_params):
    indexes_a, indexes_b = create_indexes(
        size_axis=w_l_shape, center_w_l=conv_params["center_w_l"]
    )
    stride = conv_params["stride"]
    ...
    for a in indexes_a:
        for b in indexes_b:
            ...
            dEdw_l[indexes_a.index(a)][
                indexes_b.index(b)
            ] = result  # перевод индексов в "нормальные" для
...
def conv_weights_init(shape, quantity, weights_name, dir_npy):
    try:
        ...
        print(
            ...
            weights_name,
            "подгружены",
            len(weights_matrices) * weights_matrices[0].size,
        )
    except:
        weights_matrices = []
...
def convolution_feed(
    ...
    if np.size(w_l) == 0:
        ...
        w_l = conv_weights_init(
            shape=w_shape_l,
            quantity=feature_maps * len(y_l_minus_1),
            weights_name=w_l_name,
            dir_npy=dir_npy,
        )
    for i in range(len(y_l_minus_1)):  # для всех y_l_minus_1
        for j in range(i * feature_maps, (i + 1) * feature_maps):
            ...
            x_l.append(
                convolution_feed_x_l(
                    y_l_minus_1=y_l_minus_1[i], w_l=w_l[j], conv_params=conv_params
                )
            )
    if np.size(b_l) == 0:
        # инициализация b_l (количество b_l равно числу выходов)
        b_l = conv_weights_init(
            shape=(1, 1), quantity=feature_maps, weights_name=b_l_name, dir_npy=dir_npy
        )
    x_l_final = []
    for i in range(feature_maps):  # итерация по количеству выходов
        ...
        y_l.append(
            activation_fn(x_l_final[-1], fn_name=act_fn, feed=True)
        )  # функция активации
    return y_l, w_l, b_l


def fc_multiplication(
    y_l_minus_1, w_l, w_l_name, b_l, b_l_name, neurons, act_fn, dir_npy
):
    if w_l.size == 0:
        w_l = fc_weights_init(
            shape=(y_l_minus_1.shape[1], neurons),
            weights_name=w_l_name,
            dir_npy=dir_npy,
        )
        b_l = fc_weights_init(
            shape=(1, neurons), weights_name=b_l_name, dir_npy=dir_npy
        )
...
def activation_fn(matix, fn_name, feed):
    ...
    if feed:
        if fn_name == "sigmoid":
            output_matix = 1 / (1 + np.exp(-output_matix))
        if fn_name == "relu":
        ...
        if (
            fn_name == "relu"
        ):  # relu для backprop рассчитывается исходя из того, что на вход подается y_l, а не x_l
...
def matrix2vector(matrix):
    ...
    for i in range(len(matrix)):
        reshaped_matrix = np.reshape(
            matrix[i], (1, matrix[i].shape[0] * matrix[i].shape[1])
        )
...
def convolution_backpropagation(
    y_l_minus_1, y_l, w_l, b_l, dEdy_l, feature_maps, act_fn, alpha, conv_params
):
    list_of_dEdy_l_minus_1 = []
    list_of_dEdx_l = []
    for i in range(len(y_l)):
        # сначала происходит расчет dEdx_l, то есть обратное прохождение dEdy_l через функцию активации
        list_of_dEdx_l.append(
            dEdy_l[i] * activation_fn(y_l[i], fn_name=act_fn, feed=False)
        )
